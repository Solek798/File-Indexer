...
bool Parameters::passes_filter(const boost::filesystem::directory_entry &entry) const {
    ...
    const bool check_before = map.contains("before");
    const bool check_after = map.contains("after");
    ...
    const auto before_date = check_before
                                 ? std::optional{boost::gregorian::from_simple_string(map["before"].as<std::string>())}
                                 : std::nullopt;
    const bool matches_before = check_before ? last_modified < before_date : true;

    // check after date
    const auto after_date = check_after
                                ? std::optional{boost::gregorian::from_simple_string(map["after"].as<std::string>())}
                                : std::nullopt;
